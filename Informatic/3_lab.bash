...
func ()  # Функция должна быть объявлена до того как она будет вызвана.
{
    ...
    local branch
    branch=$2  # отвечает за отступ, продлевает черту вниз, пустая строка, позволяет сделать "отскок"
    local subdirs
    ...
    do
        ...
        then 
            dir=$(( $dir + 1 ))
            func $root/$name "$branch$parent"  # сначала печать родителя, а потом выполнение функции для этой директории
        else
            file=$(( $file + 1 ))
        fi
    done
